...
// (Moved verbatim out of lab/boids-lab.html; the panel/pond split follows.)
...
(function () {
  ...
  var pond = createPond({
    ...
    assets: { journal: window.JOURNAL, book: window.BOOK },
  ...
  pond.on('atlas', function () { if (FREEZE > 0) { pond.step(FREEZE, 1 / 60); FREEZE = 0; setPaused(true); } });
  ...
  if (!pond.ok) {
    ...
    statusEl.textContent = typeof window.brush === 'undefined' ? 'p5.brush failed to load from the CDN.' : 'WebGL2 unavailable.';
  }

  window.BoidsLab = { get P() { return P; }, pond: pond, school: school, Painter: Painter, Water: Water, PondFlow: PondFlow, Journal: Journal, Drawings: Drawings, repaint: repaint, Shapes: Shapes, PATTERNS: PATTERNS,
